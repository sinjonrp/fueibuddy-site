...
name: apple-design
description: >
  Apple HIG（ヒューマンインターフェイスガイドライン）準拠の UI/UX レビュー専用スキル。
  画面・部品・フローを HIG 由来の53ガイドライン文書（references/hig/）に照らして監査し、
  深刻度付きの指摘と修正案を返す。アクセシビリティ監査（文字スケーリング・コントラスト・
  タップ領域・スクリーンリーダー）、ダークモードレビュー、プラットフォーム慣習チェック、
  App Store 審査前のデザイン確認で使う。「/apple-design」「HIGレビュー」「HIG準拠」
  「アクセシビリティ監査」「Apple的にどうか」等で発動。/pdca の Check 工程の追加視点としても使える。
  ※レビュー専用: UI の生成・配色やトークンの決定には使わない（スタイルの正本は本リポジトリの
  デザインskill〔<アプリ名>-design・存在する場合〕。矛盾する場合はデザインskillを優先し、矛盾自体を指摘として報告）。
---

# Design Review Skill

> **バディシリーズ共通での運用（導入時に追記した節。以降の本文と references/ は upstream 原文のまま）**
> - 本スキルは**レビュー（監査）専用**。UI 生成やスタイル決定には使わない。スタイルの正本は本リポジトリの
>   デザインskill（`<アプリ名>-design`・存在する場合）。両者が矛盾する場合はデザインskillを優先し、矛盾自体を指摘として報告する。
>   デザインskillが無いリポジトリでは既存コードの規約（テーマ/トークン実装）を正とする。
> - 対象（本リポジトリ）: 静的HTML公式サイト（GitHub Pages 配信・単一HTML自己完結・JSなし）。プラットフォーム慣習は「モバイル/デスクトップブラウザの Web」に読み替える。フィードバックは日本語で書く。
> - 指摘は README.md の「現在地」「次にやること」と突き合わせ、新規分のみを記録する。修正の適用は正本（`sinjonrp/FueiBuddy` の `site/`）を先に編集してから本リポジトリへコピーする（README の正本運用に従う）。
> - 出所: <https://github.com/dickwu/apple-design-skill>（main ブランチ・2026-07-20 取得）。取得時の upstream SKILL.md SHA-256: `f947d8935e981d41233e4cc3b1b4fbbd1297c87253f538fd9045de3fbe28ec8a`。変更点はこの節の追加と frontmatter の書き換えのみ。
> - 展開: 2026-07-26 に9repo（バディシリーズ7＋PayDate＋EstatePartner）へ共通展開。先行運用の実績と監査結果の実例は HomeBuddy の `docs/audit-hig-2026-07-24.md` にある。2026-08-02 に本リポジトリ（fueibuddy-site）へ追加展開（FueiBuddy 本体の同スキルからコピー・本文と references/ は同一）。
